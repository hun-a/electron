...
export default class Signup extends React.Component {
  ...
  handleOnSubmit(e) {
    ...
    if (!isValid) {
      // print errors because validation failed.
  ...
  render() {
    return (
      <form style={SIGNUP_FORM_STYLE} onSubmit={this.handleOnSubmit}>
        ...
        <div className='form-group' style={ALIGN_CENTER_STYLE}>
          <button className='btn btn-large btn-primary'>Create new account</button>
          <Link to='/login'>
            <button
              type='button'
              style={CANCEL_BUTTON_STYLE}
              className='btn btn-large btn-default'>Cancel</button>
          </Link>
        </div>
      </form>
    );
